...
@section('page-content')
    <div class="container px-12 pb-36 pt-12 lg:mt-8 max-lg:px-10 max-sm:px-5 mx-auto min-h-screen grid gap-5">
                    ...
                    <p class="text-black text-lg font-bold col-span-2 self-center">:
                        <span
                            class="badge badge-md text-white @switch($data[0]->status)
                                ...
                                @endswitch">{{ $data[0]->status == 'Menunggu Di Balas Oleh Anda.' ? 'Menunggu Di Balas Oleh User.' : $data[0]->status }}</span>
                    </p>
                </div>
        ...
        @switch($data[0]->status)
            @case('Ticket Support Telah Ditutup.')
            @case('Masalah Tidak Terselesaikan.')
            @break
            ...
            @default
                <section class="tambah-balasan card card-compact bg-white shadow-xl rounded-md">
                    <div class="card-body">
                        <div class="grid title gap-2">
                            <p class="text-lg font-medium">Tambah Balasan atau Ubah Status</p>
                            <hr class="mb-3 border-gray-300" />
                        </div>

                        <p class="text-center">Pilih Aksi:</p>
                        <div class="div flex gap-3 justify-center">

                            <form method="GET">
                                <div class="masuk">
                                    <input type="hidden" placeholder="Search" name="aksi" value="tambah_balasan" />
                                    <button type="submit"
                                        class="btn btn-sm btn-neutral text-white transition duration-300 hover:scale-90  hover:text-black hover:bg-gray-200">Tambah
                                        Balasan</button>
                                </div>
                            </form>
                            <form method="GET">
                                <div class="keluar">
                                    <input type="hidden" placeholder="Search" name="aksi" value="ubah_status" />
                                    <button type="submit"
                                        class="btn btn-sm btn-neutral text-white transition duration-300 hover:scale-90 hover:text-black hover:bg-gray-200">Ubah
                                        Status</button>
                                </div>
                            </form>
                        ...
                        <form action="{{ route('support-admin.update', $data[0]->id) }}" method="POST" class="grid gap-3"
                            ...
                            @if (Request()->aksi == 'ubah_status')
                                <div class="form-control items-center" id="ubah_status">
                                    <label class="label">
                                        <span class="label-text text-gray-500 hover:text-gray-900">Status</span>

                                    </label>
                                    <select class="select select-bordered w-full max-w-xs" name="status">
                                        <option value="Ticket Support Telah Ditutup.">
                                            Ticket Support Telah Ditutup.</option>
                                        <option value="Masalah Tidak Terselasaikan.">
                                            Masalah Tidak Terselasaikan.</option>
                                    </select>
                                </div>
                            @elseif (Request()->aksi == 'tambah_balasan')
                                <div class="form-control" id="tambah_balasan">
                                    <label class="label">
                                        <span class="label-text text-gray-500 hover:text-gray-900">Balasan Admin</span>
